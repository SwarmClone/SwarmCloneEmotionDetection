...
def cal_metrics(model, dataloader, num_classes, is_train=False, for_transformer=False):
    ...
    for batch in tqdm(
        dataloader,
        desc=f"Metrics Progress for {'Train' if is_train else 'Val'}",
        unit="batch",
    ):
        x = batch["input_ids"].to(model.device)
        if for_transformer:
            mask = batch["attention_mask"].bool().to(model.device)
